...

{
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'label' => $this->l('Currency for custom free shipping amount'),

                        'name' => 'custom_currency',

                        'desc' => $this->l('The entered amount will be interpreted in this currency and automatically converted to the currently selected shop currency.'),
...

    {
        ...
        $free_ship_from = $this->getFreeShippingThresholdCurrentCurrency();
    ...
    protected function getFreeShippingThresholdCurrentCurrency()

    {

        $targetCurrency = Currency::getCurrencyInstance((int) Context::getContext()->currency->id);



        if (Configuration::get($this->cfgName . 'custom_status')) {

            $amount = (float) Configuration::get($this->cfgName . 'custom_amount');

            $sourceCurrency = $this->getConfiguredCurrency();



            if (Validate::isLoadedObject($sourceCurrency) && Validate::isLoadedObject($targetCurrency)) {

                return (float) Tools::convertPriceFull($amount, $sourceCurrency, $targetCurrency);

            }



            return (float) Tools::convertPrice($amount, $targetCurrency);
    ...
    protected function getConfiguredCurrency()

    {

        $isoCode = Tools::strtoupper((string) Configuration::get($this->cfgName . 'custom_currency'));
        ...
        $idCurrency = (int) Currency::getIdByIsoCode($isoCode);
